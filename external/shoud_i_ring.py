...
def main(argv):
    common.LogDump( zLogger,"shouldiring is starting.",True,False,False )
    
    if common.checkPythonInterpreter(zLogger, True, False, False) == False:
    ...
    common.rdb.client_setname("alarm_shouldiring_["+str(os.getpid())+"]"  )
    ...
    device_list = common.GetSensorsStatus(zLogger,False,True,False,False)
    #print (device_list)

    #seuls les capteurs vivants peuvent declencher une alarme
    ring = False
    becoz = []
    print ("[actifs]:")
    for item in device_list:
        if device_list[item]['isalive'] == 'true':
            ...
            if device_list[item]['something_security_detected'] == True:
                ring = True
                becoz.append(device_list[item]['description'])

    #remonter un warning sur les capteurs non vivants qui devrait l'être
    print ("[inactifs]:")
    for item in device_list:
        if device_list[item]['isalive'] != 'true':
            print ( item, str(device_list[item]) )
    ...
    if ring == True:
        common.LogDump(zLogger, "!!!!! RINGING !!!!!!",True, True, False)
        common.LogDump(zLogger, "because " + str(becoz),True, True, False)
    else:
        common.LogDump(zLogger, "!![__ hush __]!!",True, True, False)
